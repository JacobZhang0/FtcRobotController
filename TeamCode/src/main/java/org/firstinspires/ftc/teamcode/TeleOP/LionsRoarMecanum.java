...
public class LionsRoarMecanum extends LinearOpMode {
    private static final int MAX_LIFT_POSITION = 4000;
    private static final int MIN_LIFT_POSITION = 0;
    private IMU imu;
    private DcMotor lift_main;
    private DcMotor lift_secondary;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        lift_secondary = hardwareMap.dcMotor.get("lift_mirrored");
        ...
        lift_secondary.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
        ...
        while (opModeIsActive()) {
            ...
            if (liftEncoderPosition > MAX_LIFT_POSITION) {
                lift_main.setPower(Math.min(0, liftPower));
                lift_secondary.setPower(Math.min(0, liftPower));
            }
            else if (liftEncoderPosition < MIN_LIFT_POSITION) {
                lift_main.setPower(Math.max(0, liftPower));
                lift_secondary.setPower(Math.max(0, liftPower));
            }
            else {
                lift_main.setPower(liftPower);
                lift_secondary.setPower(liftPower);
            }
